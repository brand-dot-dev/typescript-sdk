  ...
  inputSchema: {
    ...
    properties: {
      ...
      ticker: {
        type: 'string',
        description:
          "Stock ticker symbol to retrieve brand data for (e.g., 'AAPL', 'GOOGL', 'BRK.A'). Must be 1-15 characters, letters/numbers/dots only. Cannot be used with domain or name parameters.",
      },
      ticker_exchange: {
        type: 'string',
        description:
          'Optional stock exchange for the ticker. Only used when ticker parameter is provided. Defaults to assume ticker is American if not specified.',
        enum: [
          'AMEX',
          'AMS',
          'AQS',
